...
#ifdef SKY
/* Nk Newly added for the new requirement of power button -NK */
#define BOARD_IOCTL_SET_PWR_LED         _IOWR(BOARD_IOCTL_MAGIC, 60, BOARD_IOCTL_PARMS)
/* Murthy IOTCLs for WatchDog */
#define BOARD_IOCTL_RESET_WATCHDOG  _IOWR(BOARD_IOCTL_MAGIC, 61, BOARD_IOCTL_PARMS)
#define BOARD_IOCTL_STOP_WATCHDOG  _IOWR(BOARD_IOCTL_MAGIC, 62, BOARD_IOCTL_PARMS)
#define BOARD_IOCTL_SET_PLC_GPIO   _IOWR(BOARD_IOCTL_MAGIC, 63, BOARD_IOCTL_PARMS)
...
#ifdef SKY	
    kLedSkyHd,
    kLedPwr,  // Newly added this was required to add because of blinking requirement not there earlier
    kLedWll,
#endif /* SKY */	
...
//States For Testing LED behaviour
    kLedStateGreenOn,
    kLedStateGreenOff,
    kLedStateRedOn,
    kLedStateRedOff,
    kLedStateGreenOnRedOn,                           
    kLedStateGreenOnRedOff,                            
    kLedStateGreenOffRedOn,     
    kLedStateGreenOffRedOff
#endif //SKY    
} BOARD_LED_STATE;
...
#ifdef SKY
// Watch DOG Default count - 40 sec
#define WATCHDOG_DEFCOUNT 40
//Murthy: Comment below line to disable watch dog functionality
#define SKY_WATCHDOG_DISABLE

#ifdef SKY_FLASH_API
extern void kerSysFlashDump(unsigned int sector, int offset, int length);
extern void kerSysFlashWrite(unsigned int sector, int offset, int length);
#endif // SKY_FLASH_API
...
void kerSysSkyDiagnosticsKpWrite(void);
void kerSysSkyDiagnosticsKdieWrite(void);
#endif

extern void kerSysCfeNvRamGet(char *string, int strLen, int offset);
extern int kerSysCfeNvRamSet(const char *string, int strLen, int offset);
extern void kerSysSerialisationGet(char *string, int strLen, int offset);
extern void kerSysPublicKeyGet(char *string, int strLen, int offset);
extern int kerSysSerialisationSet(const char *string, int strLen, int offset);
#ifdef ENABLE_REPAIR_SW
extern int kerSysSectorSet(const char * string,int strLen,int sectorNumber);
#endif
extern int kerSysGetFlashSectorSize( unsigned short sector );
#ifdef ENABLE_SERIALISATION_WP
extern int kerSysSerialisationWP(void);
#else
extern int kerSysSoftUnProt(void);
extern int kerSysSoftWriteProt(void);
#endif
#endif /* SKY */
...
#ifdef SKY
extern int kerSysBootImageStateGet(void);
extern int kerSysSetSequenceNumber(int bank, int seq);
#endif
...
extern int kerSysBcmImageSet( unsigned int flash_start_addr, char *string, int size,
    int should_yield);
extern int kerSysBcmNandImageSet( char *rootfs_part, char *string, int img_size,
    int should_yield );

extern int kerWriteBootImageState(int currState, int newState);

#ifdef SKY
extern int kerSysSetBootImageState( int newState, int renumber );
#else
