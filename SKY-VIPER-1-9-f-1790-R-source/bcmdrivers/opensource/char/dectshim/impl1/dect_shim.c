...
static int dectI_dma_config(void)
{
   ...
   for ( i=0; i < DECT_DMA_NUM_CHAN; i++ )
   {
      if ( i == 0 )
      {
         ...
         pDectTxDmaDDRStartAddr[i] = (volatile unsigned long *) ALIGN_DATA( (unsigned long) dectTxDMABuff, DMA_UBUS_BYTES );
         pDectRxDmaDDRStartAddr[i] = (volatile unsigned long *) ALIGN_DATA( (unsigned long) dectRxDMABuff, DMA_UBUS_BYTES );                   
      }
      else
      {
         /* DDR */
   ...
   for ( i=0; i<DECT_DMA_NUM_CHAN; i++ )
   {
      ...
      {
         *(pDectTxDmaDDRStartAddr[i] + j) =  0x00000000; //0x01234567;
         *(pDectRxDmaDDRStartAddr[i] + j) =  0x00000000; //0x01010101;
...
#else
   dma_free_coherent( NULL, rxBufferArea, (void *)dectRxDMABuff, dmaBufHandleRx );
   dma_free_coherent( NULL, txBufferArea, (void *)dectTxDMABuff, dmaBufHandleTx );                        
#endif
