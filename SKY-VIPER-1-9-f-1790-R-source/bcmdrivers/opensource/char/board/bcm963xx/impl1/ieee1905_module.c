...
typedef struct {
  ...
  struct i5_work_queue_t    i5_udp_queue_data;
  struct i5_work_queue_t    i5_push_button_notify_data;
...
static void _i5ModulePushButtonNotify( void )
{
...
{
  struct i5_work_queue_t *i5_wq_data = container_of(work, struct i5_work_queue_t, i5_work);
  t_I5_UDP_MSG            udpMsg;
  struct msghdr           msg;
  struct iovec            iov;
  ...
  if ( i5_module_priv.i5_listener_port ) {
    ...
    to.sin_addr.s_addr = 0x7f000001;  
    to.sin_port        = i5_module_priv.i5_listener_port;
  ...
  while ( 1 )
  {
    ...
    switch ( pUdpMsg->cmd ) {
      case I5_UDP_CMD_CLIENT_REGISTER:
        ...
        i5_module_priv.i5_listener_port = pUdpHdr->source;
        break;
      ...
      case I5_UDP_CMD_PUSH_BUTTON_REGISTER:
        ...
        if ( 0 == pMsg->reg ) {
          kerSysPushButtonNotifyBind(NULL);
        }
        else {
          kerSysPushButtonNotifyBind(_i5ModulePushButtonNotify);
        }
        break;

      case I5_UDP_CMD_SES_BUTTON_TRIGGER:
#if defined(WIRELESS)
        kerSysSesEventTrigger();
#endif
        break;
