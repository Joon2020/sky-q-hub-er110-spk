...
static void moca_hw_init(struct moca_priv_data *priv, int action)
{
   ...
   u32 temp;
	...
	if (action == MOCA_ENABLE) {
		...
		/* Check for 6802 A0 chip only with Xtal mod */
		if (pMocaData->chip_id == 0x680200A0)
		{
	...
	uint32_t dst_offset, void *src, unsigned int len)
{
	...
	else
	{
		...
		mutex_lock(&priv->copy_mutex);
		if (((struct moca_platform_data *)priv->pdev->dev.platform_data)->useSpi == 1)
		{
			for(i = 0; i < len; i += 4, src++)
				*(unsigned int *)src = be32_to_cpu(*(unsigned int *)src);
...
static int  hw_specific_init( struct moca_priv_data *priv )
{
	...
	pMocaData->chip_id = MOCA_RD(0x10404000) + 0xA0;
