...
#define TAGS_REMARK(tags, r) ((uint8_t)(tags + TAGS_MAX) | (r) << 8)
    ...
    switch (TAGS_REMARK(num_tags_diff, remark))
    {
        ...
        case TAGS_REMARK(1, REMARK_VID0 | REMARK_PBIT0):
            ...
            break; /* BL_LILAC_RDD_VLAN_COMMAND_ADD_TAG + REMARK */
    ...
    for (i = 0; i < blog_rule->actionCount; i++)
    {
        ...
        switch (cmd)
        {
            ...
            case BLOG_RULE_CMD_SET_DEI:
                protoInfo("Set DEI action is not supported");
                break;
            case BLOG_RULE_CMD_COPY_DEI:
                protoInfo("Copy DEI action is not supported");
                break;
            default:
...
int blog_rule_to_vlan_action(blogRule_t *blog_rule, rdpa_traffic_dir dir, bdmf_object_handle *vlan_action_obj)
{
    ...
    *vlan_action_obj = NULL;
    ...
    /* temp workaround: since PUSH/PUSH2 use system.innertag/outertag, config them according to first flow */
    if (!(action.cmd & RDPA_VLAN_CMD_REPLACE) && (action.cmd & RDPA_VLAN_CMD_PUSH || action.cmd & RDPA_VLAN_CMD_PUSH2))
    {
        bdmf_object_handle rdpa_system;
        rdpa_system_cfg_t cfg;

        rdpa_system_get(&rdpa_system);
        rdpa_system_cfg_get(rdpa_system, &cfg);
        /* Since runner cannot handle correctly adding OUTER/INNER TPID for both single and double tagged packets
         * simultaniously, we allow either single or double tag configuration, but not both. */
        if (!system_outer_set)
        {
            system_outer_set = 1;
            cfg.outer_tpid = rdpa_mw_vlan_tag(&tags, 0)->tpid;
            if (HAS(rdpa_mw_vlan_tag(&tags, 1)->tpid))
                cfg.inner_tpid = rdpa_mw_vlan_tag(&tags, 1)->tpid;

            rdpa_system_cfg_set(rdpa_system, &cfg);
            protoDebug("First blog with TPID set, configuring global system.outer_tpid with 0x%x", cfg.outer_tpid);
        }
        bdmf_put(rdpa_system);

        if (cfg.outer_tpid != rdpa_mw_vlan_tag(&tags, 0)->tpid)
            return -1;

        if (HAS(rdpa_mw_vlan_tag(&tags, 1)->tpid) && cfg.inner_tpid == rdpa_mw_vlan_tag(&tags, 1)->tpid)
            return -1;

        protoDebug("PUSH command with same TPID as system.outer_tpid, retrying:");
        action.cmd &= ~RDPA_VLAN_CMD_TPID;
    }
