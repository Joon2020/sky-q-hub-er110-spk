...
void bcmsw_pmdio_wreg(int page, int reg, uint8 *data, int len)
{
    ...
    switch (len) {
        ...
        case 8:
             /*
               The caller gives the value as 08:07:06:05:04:03:02:01 
    ...
    if (enabled)
    {
        for (i=0; i<2; i++)
        {
            ...
            ethsw_rreg(PAGE_ARLCTRL, REG_MULTIPORT_VECTOR1 + i*16, 
                    (uint8 *)&vect, 4);
            printk("Mport Addrs: %02x:%02x:%02x:%02x:%02x:%02x, Port Map %04x\n",
                addr[0],
                addr[1],
                addr[2],
                addr[3],
...
#if defined(CONFIG_BCM96818)
#define ETHSW_PORTS_6818 8 /* No CMF on MIPS Port */
uint8 portState[ETHSW_PORTS_6818];
/*
*------------------------------------------------------------------------------
...
static int save_mib_state(void)
{
    int i, j;

    for (i = 0; i < TOTAL_SWITCH_PORTS; i++) {
        for (j = 0; j < TOTAL_MIB_COUNTERS; j++) {
            ethsw_rreg(PAGE_MIB_P0 + i, j * 4, (uint8_t*)&switch_mibs[i][j], 4);
        }
    }
    return 0;
}

static int restore_mib_state(void)
{
    int i, j;

    for (i = 0; i < TOTAL_SWITCH_PORTS; i++) {
        for (j = 0; j < TOTAL_MIB_COUNTERS; j++) {
            ethsw_wreg(PAGE_MIB_P0 + i, j * 4, (uint8_t*)&switch_mibs[i][j], 4);
        }
    }
    return 0;
}
...
static int save_switch_state(void)
{
    int i = 0;
    ethsw_reg *swreg;
    ...
    while (swreg->page != 0xFF) {
        ethsw_rreg(swreg->page, swreg->offset, (uint8 *)&swreg->val, swreg->len);
...
static int restore_switch_state(void)
{
    int i = 0;
    ethsw_reg *swreg;
    ...
    swreg = &ethsw_cfg_regs[i];
    while (swreg->page != 0xFF) {
        ethsw_wreg(swreg->page, swreg->offset, (uint8 *)&swreg->val, swreg->len);
        BCM_ENET_DEBUG("page=0x%x; offset=0x%x; val=0x%08x",
                       swreg->page, swreg->offset, swreg->val);
...
int reset_switch(void)
{
    ...
    for (i = 0; i < num_ports; i++) {
        if (ETHSW_PHY_GET_PHYID(i) != -1) {
            ethsw_phy_rreg(ETHSW_PHY_GET_PHYID(i),
               MII_BMCR, &v16);
        v16 |= BMCR_LOOPBACK;
            ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(i),
               MII_BMCR, &v16);
        }
    }
    ...
    for (i = 0; i < TOTAL_SWITCH_PORTS; i++) {
        ethsw_rreg(PAGE_CONTROL, REG_PORT_CTRL + i, &v8, 1);
        ...
        if (i < (TOTAL_SWITCH_PORTS -1)) {
            ...
            ethsw_wreg(PAGE_CONTROL, REG_CONTROL_MII1_PORT_STATE_OVERRIDE,
                       &v8, 1);
        }
    }
...
#if defined(_CONFIG_BCM_PKTCMF_MODULE)
    ...
    for (i = 0; i < num_ports; i++) {
        if (!port_in_loopback_mode[i]) {
            if (ETHSW_PHY_GET_PHYID(i) != -1) {
                ethsw_phy_rreg(ETHSW_PHY_GET_PHYID(i), MII_BMCR, &v16);
            v16 &= (~BMCR_LOOPBACK);
                ethsw_phy_wreg(ETHSW_PHY_GET_PHYID(i), MII_BMCR, &v16);
            }
        }
    }
...
int ethsw_is_switch_locked(void *ptr)
{
    ...
    // ethsw_phy_pll_up may delay by 1 msec
    // This first call, outside of the spinlock, reduces CPU hog
    ethsw_phy_pll_up(0);
#endif
...
int ethsw_get_port_buf_usage(void *ptr)
{
    ...
    ethsw_wreg(PAGE_FLOW_CTRL, REG_FC_DIAG_PORT_SEL, 
                   (uint8 *)&val16, 2);

    for (queueIdx = 0; queueIdx < NUM_EGRESS_QUEUES;)
    {
        ethsw_rreg(PAGE_FLOW_CTRL,
                       REG_FC_Q_MON_CNT + (queueIdx * 2),
                       (uint8 *)&curr_count, 2);

        /* For Debug - Start */
        ethsw_rreg(PAGE_FLOW_CTRL, REG_FC_DIAG_PORT_SEL ,
                       (uint8 *)&val16_temp, 2);
        if (val16 != val16_temp)
        {
...
{
#if defined(CONFIG_BCM96818) || !defined(SUPPORT_SWMDK)
  int i;
  uint8 v8;
  uint16 v16;
...
#endif
#endif
#if defined(CONFIG_BCM96818) || !defined(SUPPORT_SWMDK) || !defined(_CONFIG_BCM_FAP)
  uint32 v32;
#endif
#if defined(CONFIG_BCM96318)
    ETHERNET_MAC_INFO EnetInfo[BP_MAX_ENET_MACS];

    BpGetEthernetMacInfo(&EnetInfo[0], BP_MAX_ENET_MACS);
#endif
...
#else
        ethsw_port_based_vlan(GET_PORTMAP_FROM_LOGICAL_PORTMAP(portMap, 0), GET_PORTMAP_FROM_LOGICAL_PORTMAP(wanPortMap, 0), 0);
#endif
        }
#endif
    }
...
int ethsw_set_mac_hw(uint16_t sw_port, PHY_STAT ps)
{
...
#if defined(CONFIG_BCM96818)
  local_irq_save(flags);
  ethsw_rreg(PAGE_CONTROL, REG_SWITCH_MODE, &val8, sizeof(val8));
  if (val8 & REG_SWITCH_MODE_FLSH_GPON_EGRESS_Q) {
      va8 = val8 & (~REG_SWITCH_MODE_FLSH_GPON_EGRESS_Q);
      ethsw_wreg(PAGE_CONTROL, REG_SWITCH_MODE, &va8, sizeof(va8));
  }
  ...
  if (ps.lnk == 0) {
      ...
      ethsw_wreg(PAGE_FLOW_CTRL, REG_FC_DIAG_PORT_SEL, (uint8 *)&val16, 2);

      ethsw_rreg(PAGE_QOS, REG_QOS_TXQ_CTRL, &va8, 1);
      ...
      for (i = 0; i < num_queues; i++) {
          ethsw_rreg(PAGE_FLOW_CTRL, REG_FC_Q_MON_CNT + (i * 2),
                     (uint8 *)&val16, 2);
          while (val16) {
             if ((timeout--) <= 0) {
                 {
  ...
  ethsw_wreg(PAGE_CONTROL, REG_SWITCH_MODE, &val8, sizeof(val8));
  local_irq_restore(flags);
#else
  ethsw_wreg(PAGE_CONTROL, REG_PORT_STATE + sw_port, &v8, 1);
#endif
...
int enet_ioctl_ethsw_dos_ctrl(struct ethswctl_data *e)
{
    ...
    if (e->type == TYPE_GET)
    {
        if (bcm63xx_enet_isExtSwPresent())
        {
            ...
            extsw_rreg(PAGE_DOS_PREVENT_531xx, REG_DOS_CTRL, (uint8 *)&v32, 4); 
            v32 = swab32(v32);
            ...
            extsw_rreg(PAGE_DOS_PREVENT_531xx, REG_DOS_DISABLE_LRN, (uint8 *)&v8, 1);
    ...
    else if (e->type == TYPE_SET)
    {
        if (bcm63xx_enet_isExtSwPresent())
        {
            ...
            v32 = swab32(v32);
            extsw_wreg(PAGE_DOS_PREVENT_531xx, REG_DOS_CTRL, (uint8 *)&v32, 4); 
            ...
            extsw_wreg(PAGE_DOS_PREVENT_531xx, REG_DOS_DISABLE_LRN, (uint8 *)&v8, 1);
        }
        else
        {
            return BCM_E_EXISTS;
