...
#endif
#if defined(CONFIG_BCM96818)
extern MirrorCfg gemMirrorCfg[2];
uint8 defaultIPG = 0; /* Read during init */
#endif
 ...
 */
static void enet_ioctl_kernel_poll(struct net_device *dev);
int bcmeapi_ioctl_kernel_poll(struct net_device *dev, struct ethswctl_data *e)
{
    enet_ioctl_kernel_poll(dev);
...
static FN_HANDLER_RT bcm63xx_ephy_isr(int irq, void * dev_id)
{
    ...
    bcmeapiInterruptEnable(INTERRUPT_ID_EPHY);
#endif
...
FN_HANDLER_RT bcm63xx_gphy_isr(int irq, void * dev_id)
{
    ...
    bcmeapiInterruptEnable(INTERRUPT_ID_GPHY0);
    bcmeapiInterruptEnable(INTERRUPT_ID_GPHY1);
#else
    bcmeapiInterruptEnable(INTERRUPT_ID_GPHY);
#endif
#endif
    return BCM_IRQ_HANDLED;
}
...
int bcmeapi_init_dev(struct net_device *dev)
{
    ...
    bcmeapi_set_multiport_address((uint8_t*)dev->dev_addr);
...
void bcmeapi_module_init2(void)
{
    ...
    ethsw_rreg(PAGE_CONTROL, 0x0a, &defaultIPG, 1);
...
void bcmsw_set_ae_ipg(void)
{
    ...
    ethsw_wreg(PAGE_CONTROL, 0x0A, (uint8 *)&v8, 1);
}
void bcmsw_set_default_ipg(void)
{
    ethsw_wreg(PAGE_CONTROL, 0x0A, (uint8 *)&defaultIPG, 1);
}
#endif /* BCM96818 */
...
{
    int phyId = priv->EnetInfo[0].sw.phy_id[LOGICAL_PORT_TO_PHYSICAL_PORT(VPORT_TO_LOGICAL_PORT(vport))];
    int mask = (1 << VPORT_TO_LOGICAL_PORT(vport));
