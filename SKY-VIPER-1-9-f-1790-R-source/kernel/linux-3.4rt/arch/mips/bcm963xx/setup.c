...
#ifdef SKY
// SKY VIPER board only
static struct i2c_board_info sky_nfc_board_info[] __initdata = {
	{
		I2C_BOARD_INFO(NFC_NXP_NTAG_NAME, NFC_NXP_NTAG_I2C_ADDR)
	},
};
#endif
...
#ifdef SKY
	printk(KERN_INFO "Registering platform I2C devices");

	/**
	 * @brief explicit device registration since it doesn't respond for general read request by default.
	 * I enumerate it in order to avoid probe failure.
	 */
	i2c_register_board_info(NFC_NXP_NTAG_I2C_BUS, 
			sky_nfc_board_info, 
			ARRAY_SIZE(sky_nfc_board_info));
#endif
