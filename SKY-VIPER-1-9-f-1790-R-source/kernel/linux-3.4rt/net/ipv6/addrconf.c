...
static void addrconf_rs_timer(unsigned long data)
{
	...
	if (idev->cnf.forwarding && 
            ...
            ((idev->dev->priv_flags & IFF_WANDEV) && 
#if defined(CONFIG_SKY_IPV6)
		(!(idev->dev->priv_flags & IFF_PPP) && !(idev->dev->priv_flags & IFF_BCM_VLAN) && !(idev->dev->priv_flags & IFF_SKY_WANOE)))
#else
...
#if defined(CONFIG_SKY_IPV6)
		((ifp->idev->dev->priv_flags & IFF_PPP) || (ifp->idev->dev->priv_flags & IFF_BCM_VLAN)
                 || (ifp->idev->dev->priv_flags & IFF_SKY_WANOE))
#else
		(strchr(ifp->idev->dev->name, '.'))
#endif /* CONFIG_SKY_IPV6 */
		) ) &&
#else
