...
void br_send_config_bpdu(struct net_bridge_port *p, struct br_config_bpdu *bpdu)
{
	...
	if (!p || !p->br || p->br->stp_enabled != BR_KERNEL_STP)
		return;
...
void br_send_tcn_bpdu(struct net_bridge_port *p)
{
    ...
    /*
	 * must check p against NULL here, since the port is being looked up
	 * by br_get_port(). This function returns NULL if there's no root port found
	 * in the bridge.
     */
	if (!p || !p->br || p->br->stp_enabled != BR_KERNEL_STP)
		return;
