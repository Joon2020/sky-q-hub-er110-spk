...
int br_handle_frame_finish(struct sk_buff *skb)
{
	 ...
	 */
	if (automesh_hacks_enabled && (p->state == BR_STATE_BLOCKING || p->state == BR_STATE_LISTENING || p->state == BR_STATE_LEARNING)) {
	...
	*/
...
#if defined(CONFIG_BCM_KF_EXTSTATS)
	{
		...
		if ((dst != NULL) && (!dst->is_local))
		{
			...
			struct net_device *root_dst_dev_p = NULL;
			...
			if (unlikely(dst->dst == NULL))
				goto next;

			if (unlikely(src->dst == NULL))
				goto next;

			root_dst_dev_p = dst->dst->dev;
...
forward:
...
#if defined(CONFIG_SKY_MESH_SUPPORT) && !defined(DISABLE_AUTOMESH)
	...
	case BR_STATE_BLOCKING:
		if (p->state == BR_STATE_BLOCKING || p->state == BR_STATE_LISTENING || p->state == BR_STATE_LEARNING) {
    ...
    case BR_STATE_DISABLED:
        ...
        if(!netif_carrier_ok(p->dev))
            goto drop;
        //drop non-automesh packets
        if(skb->protocol != 0x7373)
            goto drop;
