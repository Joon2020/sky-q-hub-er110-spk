...
#ifdef CONFIG_SKY_MESH_SUPPORT
#define NETDEV_CHANGEBLOCK  0x0100
#endif
...
#ifdef CONFIG_SKY_MESH_SUPPORT
/**
 * netif_blocked_on - mark device as blocked.
 * @dev: network device
 *
 * Mark device as blocked (as per RFC2863).
 *
 * The blocked state indicates that the relevant interface is not
 * actually in a condition to pass packets.
*/
static inline void netif_blocked_on(struct net_device *dev)
{
...
/**
 * netif_blocked_off - set device as not blocked.
 * @dev: network device
 *
 * Device is not in blocked state.
 */
static inline void netif_blocked_off(struct net_device *dev)
{
   if (test_and_clear_bit(__LINK_STATE_BLOCKED, &dev->state))
       linkwatch_fire_event(dev);
}

/**
 * netif_blocked - test if carrier present
 * @dev: network device
 *
 * Check if carrier is present on device
 */
static inline int netif_blocked(const struct net_device *dev)
{
   return test_bit(__LINK_STATE_BLOCKED, &dev->state);
}
#endif /*CONFIG_SKY_MESH_SUPPORT*/
/**
