...
/*
 * Flash mapping code for BCM963xx board flash memory
 *
...
static int __init init_brcm_physmap(void)
{
	...
	rootfs_addr = (unsigned int)simple_strtoul(pTag->rootfsAddress, NULL, 10) + BOOT_OFFSET;
	kernel_addr = (unsigned int)simple_strtoul(pTag->kernelAddress, NULL, 10) + BOOT_OFFSET;
