...
int ReadBPCMRegister(int devAddr, int wordOffset, uint32 *value)
{
	...
	int status;
#ifndef _CFE_	
	if((devAddr == PMC_ADDR_APM) && (UtilGetChipRev() <= 1))
...
{
    TCommand rsp;
    int status = SendCommand(cmdGetDieTemp, 0, 0, 0, 0, &rsp);
...
EXPORT_SYMBOL(GetDieTemperature);

int GetVoltage(void)
{
    TCommand rsp;
    int status = SendCommand(cmdGetAVSVoltage, 0, 0, 0, 0, &rsp);
