...
#define RESERVED_LEN    (TAG_LEN - TAG_VER_LEN - SIG_LEN - SIG_LEN_2 - CHIP_ID_LEN - BOARD_ID_LEN - \
                        (4*IMAGE_LEN) - (3*ADDRESS_LEN) - (4*FLAG_LEN) - (2*TOKEN_LEN) - IMAGE_VER_LEN)
...
typedef struct _FILE_TAG
{
	...
	// [TW]
	// 4 characters - doesn't seem much enough. Especially
	// when considering that the actual number here is stored as string
	// excluding the termination character that gives biggest number of 999
	// increased the field length to 6 characters and shortened the reversed area
	// in order to achieve maximum sequence number value of 99999 (not considering the maximum range of int16)
    char imageSequence[FLAG_LEN * 3];   // incrments everytime an image is flashed
...
typedef struct _WFI_TAG
{
    ...
    unsigned int wfiReserved;
} WFI_TAG, *PWFI_TAG;

#define CRC32_INIT_VALUE 0xffffffff /* Initial CRC32 checksum value */
#define CRC_LEN 4
#ifdef SKY	
	/* Murthy  SKY_IHR, uses Reserved filed to store image type

	SKY_IHR_WHOLE_IMAGE - whole image  (CFE+SERDATA+ROOTFS+KERNEL) for 6362 board (SkyHub)
	SKY_IHR_PARTIAL_IMAGE - partial image  (ROOTFS+KERNEL) for 6362 board (SkyHub)

	2 new types (Below) are added to avoid accidental download of SkyHub onto Sky VDSL router and vice versa 
	
	SKY_102_WHOLE_IMAGE - whole image  (CFE+SERDATA+ROOTFS+KERNEL) for 63268 board (Sky VDSL)
	SKY_102_PARTIAL_IMAGE - partial image  (ROOTFS+KERNEL) for 63268 board (Sky VDSL)
	
	*/

typedef enum {

#if defined(CONFIG_SKY_MODEL_SR101) || defined(CONFIG_SKY_MODEL_ER110) || defined(CONFIG_SKY_MODEL_EE120)
	SKY_IHR_WHOLE_IMAGE=0,
	SKY_IHR_PARTIAL_IMAGE=1,
#else
	SKY_IHR_WHOLE_IMAGE=2,
	SKY_IHR_PARTIAL_IMAGE=3,
#endif
...
#if defined(CONFIG_SKY_ETHAN) || defined(SKY_EXTENDER)
    ROUTER_MODEL_SR101=0, 
    ROUTER_MODEL_SR102=1,
#else
    /*
	 * [TW]
	 * Unfortunately in order to be compatible with legacy images this is not supported
	 * The routerModel field does not exist in the old code and is always memsetted to 0
     */
    ROUTER_MODEL_SR101=0, 
    ROUTER_MODEL_SR102=0,
#endif

    ROUTER_MODEL_ETHAN=2,
    ROUTER_MODEL_EE120=3,
}eRouterModel;


typedef struct  _SKY_IHR_IMAGE_TAG
{
	unsigned long imageType;
	unsigned long nvRamVersion;
	unsigned long routerModel;
	unsigned long reserved2;
	unsigned long reserved3;
	
}SKY_IHR_IMAGE_TAG;

#define SKY_IMG_TAG_LEN sizeof(SKY_IHR_IMAGE_TAG)

// [TW]
// this is a dirty hack to force flashing the signature along with the image
#define SKY_SIGNATURE_SIZE 160
