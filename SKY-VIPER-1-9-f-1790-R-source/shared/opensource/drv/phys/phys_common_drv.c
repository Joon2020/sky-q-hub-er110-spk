...
#define D_NUMBER_OF_SUPPORTED_PHYS		8
...
static S_PhyCache 	phyAddrCache[D_NUMBER_OF_SUPPORTED_PHYS] = {[0 ... (D_NUMBER_OF_SUPPORTED_PHYS-1)] = {0,0,0} };
static uint32_t 	PhyCacheInitialized = 0;
static uint32_t 	boardParamsPorts	   = 0;
...
static inline void fillBpPhyCache(void)
{
	...
	bitcount(boardParamsPorts,pE[0].sw.port_map);

	for( iter = 0 ; iter < boardParamsPorts; iter++)
	{
		phyAddrCache[iter].phyId 	= pE[0].sw.phy_id[iter] & BCM_PHY_ID_M;
		phyAddrCache[iter].mdio  	= pE[0].sw.phy_id[iter] & PHY_EXTERNAL  ? MDIO_EXT:MDIO_EGPHY;
