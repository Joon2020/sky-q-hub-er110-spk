...
void flash_init_info(const NVRAM_DATA *nvRam, FLASH_ADDR_INFO *fInfo)
{
    ...
    int totalSize = 0;
    int auxFsSize = 0;
#if !defined(SKY_IHR_CFE) || defined(CONFIG_SKY_ETHAN)
    int psiStartAddr = 0;
    int spStartAddr = 0;
    int usedBlkSize = 0;
    int needBytes = 0;
...
#if defined(SKY_IHR_CFE) && !defined(CONFIG_SKY_ETHAN)
	i = 0;
    fInfo->flash_sky_scratch_pad_start_blk = SKY_IHR_SP_SECTOR;
    fInfo->flash_sky_scratch_pad_number_blk = 1;
    fInfo->flash_sky_scratch_pad_length = flash_get_sector_size(SKY_IHR_SP_SECTOR) - SKY_IHR_SP_OFFSET;
    fInfo->flash_sky_scratch_pad_blk_offset =  SKY_IHR_SP_OFFSET;
    fInfo->flash_meta_start_blk = SKY_IHR_SP_SECTOR;
    ...
    fInfo->flash_sky_scratch_pad_start_blk = SKY_SP_SECTOR;
    fInfo->flash_sky_scratch_pad_number_blk = 1;
    fInfo->flash_sky_scratch_pad_length = flash_get_sector_size(SKY_SP_SECTOR) - SKY_SP_OFFSET;
    fInfo->flash_sky_scratch_pad_blk_offset =  SKY_SP_OFFSET;


	/*  NVRM  which is a copy of the CFE_NVRAM    and SKY_SCRATCH_PAD is stored in last sector
	So Total Size and number of blocks is calculated excluding last sector  
	*/
	totalSize -= flash_get_sector_size(SKY_SP_SECTOR);
	totalBlks -= 1;
#endif /* SKY */	
    /*
    ...
    */
	...
	psiStartAddr = totalSize - (fInfo->flash_persistent_length + fInfo->flash_sky_scratch_pad_length);
#else /* SKY */
...
#ifndef CONFIG_SKY_ETHAN
    fInfo->flash_persistent_blk_offset =  usedBlkSize - (fInfo->flash_persistent_length + fInfo->flash_sky_scratch_pad_length);
#else
    ...
    */
    ...
    /* This Modification is done to support older CFE in which AUXFS percent 
    *  not defined.
    */
#ifdef CONFIG_SKY_ETHAN
...
#else
   if ( (DEFAULT_AUXFS_PERCENT != 0)
    && (DEFAULT_AUXFS_PERCENT <= MAX_AUXFS_PERCENT))
#endif
    {
...
#else
        auxFsSize = (totalSize * (int)DEFAULT_AUXFS_PERCENT)/100; /*Always take DEFAULT_AXFS_PERCENT , it will be 10 */
#endif        
        /* JFFS_AUXFS offset */
...
#ifdef SKY	
	printk("fInfo->flash_sky_scratch_pad_start_blk = %d\n", fInfo->flash_sky_scratch_pad_start_blk);
	printk("fInfo->flash_sky_scratch_pad_number_blk = %d\n", fInfo->flash_sky_scratch_pad_number_blk);
	printk("fInfo->flash_sky_scratch_pad_length = 0x%x\n", fInfo->flash_sky_scratch_pad_length);
	printk("fInfo->flash_sky_scratch_pad_blk_offset = 0x%x\n\n", (unsigned int)fInfo->flash_sky_scratch_pad_blk_offset);
#endif /* SKY */
