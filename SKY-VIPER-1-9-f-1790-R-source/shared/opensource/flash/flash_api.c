...
#if (defined(_BCM96838_) || defined(CONFIG_BCM96838)) && (INC_NAND_FLASH_DRIVER==1)
...
int flash_init(void)
{
    ...
    unsigned long straps = BL_CR_CONTROL_REGS_STRPL_READ(straps);
    if(straps & (1<<16))
        type = FLASH_IFC_SPI;
    else
        type = FLASH_IFC_NAND;
	...
	unsigned int bootsel = GPIO->strap_bus;
	if( (((bootsel & MISC_STRAP_BUS_BOOT_SEL_MASK)>>MISC_STRAP_BUS_BOOT_SEL_SHIFT) == MISC_STRAP_SPI_3BYTE_ADDR_MASK) ||
		(((bootsel & MISC_STRAP_BUS_BOOT_SEL_MASK)>>MISC_STRAP_BUS_BOOT_SEL_SHIFT) == MISC_STRAP_SPI_4BYTE_ADDR_MASK)	)
			type = FLASH_IFC_SPI;
		else
	if( (((bootsel & MISC_STRAP_BUS_BOOT_CFG_MASK)>>MISC_STRAP_BUS_BOOT_SEL_SHIFT) == MISC_STRAP_SPI_4BYTE_ADDR_MASK) &&
		(((bootsel & MISC_STRAP_BUS_PAGE_SIZE_MASK)>>MISC_STRAP_BUS_PAGE_SIZE_SHIFT) & MISC_STRAP_BUS_SPI_NAND_BOOT)	)
 ...
 * Function Name: flash_write_protect_serialisation_sectors
 * Description  : Write protect the serialisation sectors.
 * Returns      : FLASH_API_OK or FLASH_API_ERROR
 ***************************************************************************/
#ifdef ENABLE_SERIALISATION_WP
int flash_write_protect_serialisation_sectors()
{
    return( (g_flash_info)
            ? (*g_flash_info->fn_flash_write_protect_serialisation_sectors) ()
            : FLASH_API_ERROR );
}
#else
int flash_software_unprotect()
{
    return( (g_flash_info)
            ? (*g_flash_info->fn_spi_flash_software_unprotect) ()
            : FLASH_API_ERROR );
}

int flash_software_write_protect()
{
    return( (g_flash_info)
            ? (*g_flash_info->fn_spi_flash_software_write_protect) ()
            : FLASH_API_ERROR );
}

int flash_is_write_protected() {
    return( (g_flash_info)
            ? (*g_flash_info->fn_spi_flash_is_write_protected) ()
            : FLASH_API_ERROR );
}
#endif
#endif /* SKY */
...
int flash_get_flash_type(void)
{
    return( (g_flash_info)
        ...
        : FLASH_API_ERROR );
#else
        /* For Linux, if flash device not detected, assume NAND */
        : FLASH_IFC_NAND );
#endif
