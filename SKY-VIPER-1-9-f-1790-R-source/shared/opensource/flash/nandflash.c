...
#if !defined(_BCM96328_) && !defined(_BCM96838_) && !defined(_BCM963381_) 
    ...
    int len)
{
    ...
    do
    {
        ...
        temp = nandflash_read_page(pchip,start_addr,buffer,size);

        if (temp == FLASH_JFFS2_ERROR)
        {
           ret = FLASH_JFFS2_ERROR;
           break;
        }
        if (temp != FLASH_API_OK)
        {
           ret = FLASH_API_ERROR;
           break;
        }
