...
#ifdef SKY
/** Retrieve the time value from the XSI used in TR69.
 *
 * @param buf        (IN) Buffer holding the time in UTC format (used in TR69 XSI)
 * @param timeInfo   (OUT) Pointer to time structure whose values are to be updated
 *
 * @return CmsRet enum, CMSRET_SUCCESS: Parsing successful
 *                      CMSRET_INVALID_ARGUMENTS: Failure due to invalid input arguments
 *                      CMSRET_INTERNAL_ERROR: Failure in parsing the given string
 */
CmsRet cmsTms_parseXSIDateTime(char *buf, struct tm *timeInfo);
#endif /* SKY */
