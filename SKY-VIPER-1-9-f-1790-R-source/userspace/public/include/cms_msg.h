...
typedef enum 
{
   ...
   CMS_MSG_WLAN_LINK_STATUS_CHANGED             = 0x10002655, /**<APSTA link event to ssk to update Wifi Wan status */
...
#ifdef SKY
   CMS_MSG_LOG_SITEBLOCK                = 0x20002700, /**< blocking */
   CMS_MSG_WAN_MODE_CHANGED             = 0x20002701, /**< the WAN MODE (DSL only, Etherwan only, auto detect) has changed */
   CMS_MSG_ADD_PPPOE_SERVICE			= 0x20002702,
   CMS_MSG_ADD_PPPOA_SERVICE	   		= 0x20002703,
   CMS_MSG_SKY_WAN_CONNECT              = 0x20002800, /* Added by sky, to connect existing wan connections */
   CMS_MSG_SKY_WAN_DISCONNECT           = 0x20002801, /* Added by sky, to disconnect existing wan connections */
   CMS_MSG_SKY_WAN_TRAINING             = 0x20002802, /* Added by sky, to disconnect existing wan connections */
   CMS_MSG_SKY_WAN_DETECTED             = 0x20002803,
   CMS_MSG_FLASH_WRITE                  = 0x20002900, /**< blocking */ 
   CMS_MSG_UPNP_BYEBYE                  = 0x20002905, /* This is for UPNP BYEBYE */
   CMS_MSG_SKYTEST_DONE                 = 0x20002908,
   CMS_MSG_WAN_CONNECTION_AUTH_FAIL     = 0x20002909,
   CMS_MSG_REFRESH_ATTACHED_DEVICES     = 0x2000290a,
   CMS_MSG_WLAN_ASSOC_DISACCOC          = 0x2000290b,
   CMS_MSG_LSNET_COMPLETED              = 0x2000290c,
   CMS_MSG_HALTH_MONITOR_CLEAR_STATS	= 0x2000290d,
   CMS_MSG_WLAN_ASSOC                   = 0x20002910,
   CMS_MSG_WLAN_DISASSOC                = 0x20002911,
   CMS_MSG_LSNET_HOST_INFO              = 0x20002912, /**< Sent from lsnet to ssk to inform of lan host add/delete */
   CMS_MSG_SKY_REFRESH_MACFLT           = 0x20002913, /* Allowed MAC address list refresh messages*/
   CMS_MSG_MOVE_ETHPORT_TO_BRIDGE       = 0x20002914, /* Sent from SkyDhcpc to SSK, to remove WAN ethernet interface and  move hte port to BRIDGE */
#ifdef SKY_PLC
   CMS_MSG_SKY_PLC_SIMPLE_CONNECT       = 0x20002915, /* Sent from httpd to plchost to start simple connect procedure */
   CMS_MSG_SKY_PLC_RANDOMIZE_NMK        = 0x20002916, /* Sent from httpd to plchost to start randomize NMK procedure */
   CMS_MSG_SKY_PLC_POWER_ON             = 0x20002917, /* Sent from rcl to plchost to power on procedure */
   CMS_MSG_SKY_PLC_POWER_OFF            = 0x20002918, /* Sent from rcl to plchost to power OFF procedure */
   CMS_MSG_SKY_PLC_UPDATE_NMK           = 0x20002919, /* Sent from rcl to plchost to UPDATE NMK procedure */
#endif //SKY_PLC

	CMS_MSG_UPNP_PCP_SEND_REQ           = 0x20002920, /* Sent from non-UPnP source like IP firewall to UPnP for sending PCP requests for the port mappings */
	CMS_MSG_MAL_WPS_PBC                 = 0x20002921,
	CMS_MSG_MAL_WPS_PIN                 = 0x20002922,

#ifdef SKY_IPV6
    CMS_MSG_DHCP6C_PPP_START            = 0x20002923, /* Trigger to start Dhcpv6 client for PPP connection as IPV6CP needs to be completed before */
#endif /* SKY_IPV6 */

#ifdef SKY_EXTENDER
    CMS_MSG_CONNECTED_TO_MESH           = 0x20002924, /* Sent from sky lan subsystem to SNTP to report mesh connection */
    CMS_MSG_IPADDR_CHANGE               = 0x20002925, /* Initimates assignment/change of IP address at br0 LAN interface */
#ifdef SKY_TR69C_XMPP_SUPPORT
    CMS_MSG_TR69C_XMPP_CONFIG_CHANGED   = 0x20002926, /* Sent from rcl handler to handle change in TR69c XMPP config params */
    CMS_MSG_TR69C_XMPP_RETRY            = 0x20002927, /* Delayed message ID used for setting up XMPP Connection retry timer */
#endif /* SKY_TR69C_XMPP_SUPPORT */
    CMS_MSG_SKY_GW_PING_STATUS          = 0x20002928, /* Sent from sky gateway pinger to LAN subsystem event handler */
#endif //SKY_EXTENDER
#ifdef SKY_TR69C_ANTISPOOF_SUPPORT
    CMS_MSG_SKY_LOCAL_TOKEN_TIMEOUT     = 0x20002929, /* Used for handling timeout for the TR069 local token */
#endif /* SKY_TR69C_ANTISPOOF_SUPPORT */

#ifdef SKY_REBOOT_DIAGNOSTICS
    /*
	 * a separate msg id has been created to distribute reboot notification
	 * because CMS_MSG_REBOOT_SYSTEM when received by smd is not relayed anywhere else.
	 * smd executes reboot immediatelly and sends the kill signal to all slave processes.
	 *
	 * In order for the RTM to be notified about any custom reboot event, please prepare
	 * SkyMsgRtmRebootNotif and send it to RTM before informing smd
	 *
     */
	CMS_MSG_SKY_RTM_REBOOT_NOTIF		= 0x2000292a,
#endif
#ifdef SKY_DIAGNOSTICS
	CMS_MSG_GET_DHCP_MSG_COUNT			= 0x2000292b,
	CMS_MSG_DUMP_DHCP_MSG_STATS			= 0x2000292c,
	CMS_MSG_START_DHCP_MSG_STATS		= 0x2000292d,	
	CMS_MSG_STOP_DHCP_MSG_STATS			= 0x2000292e,
#endif //SKY_DIAGNOSTICS
	CMS_MSG_SHOW_APPS_BUFQ_STATS		= 0x2000292f,
	CMS_MSG_UPDATE_WAN_LINK_LED         = 0x20002930,
	CMS_MSG_IS_ANY_WAN_LINK_UP          = 0x20002931,

#ifdef SKY_MESH_SUPPORT
    CMS_MSG_MESH_INIT_COMPLETE          = 0x23000001,
#ifdef SKY_EXTENDER
    CMS_MSG_ACCESS_POINT_STATUS         = 0x23000002,
#endif // SKY_EXTENDER
#endif // SKY_MESH_SUPPORT
    CMS_MSG_KEEP_ALIVE                  = 0x23000003,
#endif //SKY
...
typedef struct cms_msg_event_body
{
    char macAddr[MAC_STR_LEN + 1];
} CmsWlEvtBody;

typedef struct cms_msg_event_msg
{
    CmsMsgHeader hdr;
    CmsWlEvtBody body;
} CmsWlEvtMsg;
...
#ifdef SKY_REBOOT_DIAGNOSTICS
/**
 * @brief reboot notification body
 */
typedef struct _SkyMsgRtmRebootNotifBody {
	UINT32 timestamp;
	UINT32 subcode;
	char descr[BUFLEN_512];
} SkyMsgRtmRebootNotifBody;


/**
 * @brief reboot notification message format
 */
typedef struct _SkyMsgRtmRebootNotif
{
    CmsMsgHeader head;
	SkyMsgRtmRebootNotifBody body;
} SkyMsgRtmRebootNotif;
#endif
...
typedef struct
{
   UBOOL8 addressAssigned; /**< Have we been assigned an IP address ? */
   UBOOL8 isExpired; /**< Is the lease time expired ? */
   UBOOL8 isDSL; /**< Is the lease time expired ? */
   UBOOL8 lommondFound;
   UINT32 ip;   /**< New IP address, if addressAssigned==TRUE */
   UINT32 mask; /**< New netmask, if addressAssigned==TRUE */
   UINT32 router;    /**< New gateway, if addressAssigned==TRUE */
   UINT32 dnsServer; /**< New dns Server, if addressAssigned==TRUE */
   UINT32 dnsServer1; /**< New dns Server, if addressAssigned==TRUE */
   UINT32 upstreamCurrRate;  // upstream rate of wanoe connection
   UINT32 downstreamCurrRate; // downstream rate of wanoe connection
   char dhcpcInterface [BUFLEN_32];
} SkyDhcpcStateChangedMsgBody;

typedef struct
{
    char interface[12];
}SkyDhcpcWanLedMsgBody;

/** Data body for CMS_MSG_WAN_LINK_UP nad CMS_MSG_WAN_LINK_DOWN message types.
 *
 */
typedef struct
{
   UBOOL8 isDSL; /**< The interface is an ATM interface (else ethernet) */
   UBOOL8 dhcpcRequired; /* requires the services of the SKY DHCPC client */
   char layer2InterfaceName [BUFLEN_16];
} linkUpDownMsgBody;

typedef struct
{
   char username[BUFLEN_64]; /**< New netmask, if pppState==BCM_PPPOE_CLIENT_STATE_UP */
   char password[BUFLEN_64];    /**< New gateway, if pppState==BCM_PPPOE_CLIENT_STATE_UP */
} PppoaServiceCfgMsgBody;
...
#ifdef SKY   
   char clientid[BUFLEN_256]; // vendor id - used for anytime devices
#endif /* SKY */   
...
#ifdef SKY
/* Structure encompassing CMS message header and body for declarations on stack.
 *
 * Any new code for sending CMS messages is STRICTLY expected to use this structure
 * instead of the existing convention of using char buffer for "CmsMsgHeader" and
 * data. This is to avoid issues due to turning on "strict-aliasing" compiler option,
 * wherein the result of typecasting "char *" pointer to "CmsMsgHeader *" pointer is
 * unpredictable, owing to possible compiler optimizations (as compiler is not
 * expected to consider this as a valid type conversion).
 *
 * The structure can be extended for new message types by adding their
 * respective body structure as member of the union (body) in the below structure.
 */
typedef struct
{
    CmsMsgHeader hdr;
    union
    {
        RegisterDelayedMsgBody delayedMsg;
        SkyDhcpcWanLedMsgBody dhcpcSetLedMsg;
    } body;
} SkyCmsMsgInfo;
#endif /* SKY */
