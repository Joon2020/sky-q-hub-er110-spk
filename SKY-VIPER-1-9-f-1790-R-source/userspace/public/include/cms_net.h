...
/** File used for sharing prefix details such as preferred, valid lifetimes
 *  between CMS and radvd application
 */
#define RADVD_PREFIX_CONF_FILE    "/var/radvd_prefix.conf"

/** Time (in ms) that a process will wait before trying to re-acquire lock
 *  on the Radvd conf file (here, radvd prefix conf file)
 */
#define RADVD_FILE_LOCK_TIMEOUT   (60000)
