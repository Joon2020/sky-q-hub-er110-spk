...
/** Validate the given config file and return the image format of the given image.
 * 
 * The image is a config file.
 *
 * @param imagePtr (IN) image to be parsed.
 * @param imageLen (IN) Length of image in bytes.
 * @param msgHandle (IN) message handle from the caller.
 ...
 * @return CmsImageFormat enum.
 */
CmsImageFormat cmsImg_validateConfigFile(const char *imagePtr, UINT32 imageLen, void *msgHandle);

/* NK Added prototype here */
void generic_led_update(unsigned int led_state,int which_led);

/** Identify an Image is Full or Partial in SKY build.
 *
 * The image can be a full (Bootloader ,Cfe,Kernel,App),or partial(Kernal,App)image.
 * This function will Identify that the image is full or partial.
 *
 * @param imagePtr (IN) image to be parsed.
 * @param imageLen (IN) Length of image in bytes.
 *
 * @return UBOOL8 TRUE or FALSE.
 */
