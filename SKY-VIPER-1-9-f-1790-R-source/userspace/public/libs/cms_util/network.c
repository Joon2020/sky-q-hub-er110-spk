...
UBOOL8 cmsNet_isAddressOnLanSide(const char *ipAddr)
{
   ...
   else
   {
      ...
      {
         cmsLog_debug("ipaddress [%s] lanAddr6[%s].", ipAddr, lanAddr6);
...
UBOOL8 cmsNet_areIp6AddrEqual(const char *ip6Addr1, const char *ip6Addr2)
{
   ...
   {
      cmsLog_debug("cmsUtl_parsePrefixAddress returns error. ret=%d", ret);
      return FALSE;
   }
   ...
   {
      cmsLog_debug("Invalid address1=%s", address1);
      return FALSE;
   }
   if (inet_pton(AF_INET6, address2, &in6Addr2) <= 0)
   {
      cmsLog_debug("Invalid address2=%s", address2);
      return FALSE;
   }
