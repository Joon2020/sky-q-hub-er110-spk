...
/* Length of buffer to hold strings. Memory to hold 64 characters + 1 null-termination character. */
#define STR_BUFFER_LEN (BUFLEN_64 + 1)
CmsRet cmsUtl_alphaNumStrToNum (const char *alphaStr, UINT64 *convNum)
{
    CmsRet ret = CMSRET_SUCCESS;
    UINT8 len = 0;               /* Maximum length of string is considered as 64 */
    UINT8 charIndex = 0;
    char charVal = 0;
    char oldStr[STR_BUFFER_LEN] = {'\0'};
    char convStr[STR_BUFFER_LEN] = {'\0'};

    cmsLog_debug ("Enter %s", __FUNCTION__);

    if ((NULL == alphaStr) || (NULL == convNum))
    {
    ...
    len = ((cmsUtl_strlen (alphaStr) < (STR_BUFFER_LEN - 1))
           ? cmsUtl_strlen (alphaStr) : (STR_BUFFER_LEN - 1));
    for (charIndex = 0; charIndex < len; charIndex++)
    {
        charVal = alphaStr[charIndex];
        if (isdigit (charVal))
        {
            /* Use the character as such */
            snprintf (convStr, len, "%s%c", oldStr, charVal);
        }
        else
        {
            /* Use ASCII value of the character */
            snprintf (convStr, len, "%s%d", oldStr, charVal);
        }
        cmsUtl_strncpy (oldStr, convStr, len);
    }

    cmsLog_debug ("Final conv str: %s", convStr);
    if (CMSRET_SUCCESS != (ret = cmsUtl_strtoul64 (convStr, NULL, 10, convNum)))
    {
        cmsLog_error ("Failed to convert string [%s] to integer, ret: %d", convStr, ret);
    }
    else
    {
        cmsLog_debug ("Conv numeric value : %llu", *convNum);
    }
...
} /* End of cmsUtl_alphaNumStrToNum() */
...
UBOOL8 cmsUtl_isULA(const char *address, UBOOL8 isPrefixOnly)
{
   struct in6_addr in6Addr;
   UINT32 plen;
   char   addr[BUFLEN_48];
   ...
   if (isPrefixOnly)
   {
       /* Prefix format is xxxx:xxxx:xxxx:xxxx::/64
        * After separating prefix length from it, the address part (addr)
        * contains xxxx:xxxx:xxxx:xxxx:: which is
        * (4*4 (address chracters) + 5 (colons) = 21)
        */
       addrLen = 21;
   }
   else
   {
       /* When the complete ULA address is provided for validation, the format
        * is xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx/64
        * After separating prefix length from it, the address part (addr)
        * contains xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx:xxxx which is
        * (8*4 (address chracters) + 7 (colons) = 39)
        */
       addrLen = 39;
   }

   /* Currently IANA assigned ULA prefix range is FC00::/7.
    * The complete prefix needs to be entered i.e., :: is not accepted in the prefix as it ends with ::/64
    * The allowed prefix length is 64 only.
    */
   cmsLog_debug ("Parsed addr: %s, len:%d", addr, strlen (addr));
   /*Don't need to check the prefix length here*/
   if (((in6Addr.s6_addr[0] & 0xfe) == 0xfc) && (strlen (addr) == addrLen))
   {
      return TRUE;
   }
