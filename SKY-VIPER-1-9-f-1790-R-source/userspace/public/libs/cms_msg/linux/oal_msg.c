...
#define SKY_SMD_SAFE_SOCKET_WRTITE
#ifdef SKY_SMD_SAFE_SOCKET_WRTITE
#include <sys/ioctl.h>
#endif
...
/*
* ER-1244:
* SMD is the core daemon for all other processes in the system.
* So don't attempt to write its response back to client app
* when client socket is NOT ready for write. One of the reasons
* could be that the OUTBUFQ might be FULL because the client
* process might not have read the socket buffer.
* Use select call to determine status of the socket and write
* the response safely.
* Whenever the SMD is not able to write the response,
* use TIOCOUTBUFQ ioctl to query out buf queue size and print
* error message.
*/
   struct timeval tv;
   fd_set writeFds;
   ...
   if(rc < 0 )
   {
      cmsLog_error("SMD select error!!! ret %d errno %d\n",rc,errno);
   }
   else if(FD_ISSET(fd, &writeFds))
   {
#endif /*SKY_SMD_SAFE_SOCKET_WRTITE*/
   ...
   {
       /*The client app's socket pipe  is not ready for write. Print the error message here*/
       const CmsEntityInfo *eSrcInfo;
       const CmsEntityInfo *eDstInfo;
       char *sName,*dName;
       CmsEntityId eid;
       int pending=0;
       int select_errno = errno;
       ioctl(fd,TIOCOUTQ,&pending);
       eid = GENERIC_EID(buf->dst);
       eDstInfo = cmsEid_getEntityInfo(eid);
       eid = GENERIC_EID(buf->src);
       eSrcInfo = cmsEid_getEntityInfo(eid);
       ...
       if(eDstInfo==NULL)
          ...
          dName = eDstInfo->name;
       cmsLog_error("SMD can't write!!! socket is NOT ready for WRITE. Src - %s(%d) Dst - %s(%d),msg 0x%x , rsp 0x%x , OutQ pend buf %d select_errno %d ",sName,GENERIC_EID(buf->src),dName,GENERIC_EID(buf->dst),buf->type,buf->wordData,pending,select_errno);
   }
#endif /*SKY_SMD_SAFE_SOCKET_WRTITE*/
   return ret;
}
