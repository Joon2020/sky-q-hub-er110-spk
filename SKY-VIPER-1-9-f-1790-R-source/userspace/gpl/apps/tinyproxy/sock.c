 ...
 */
static int
bind_socket (int sockfd, const char *addr, int family, struct addrinfo *res)
{
...
int opensock (const char *host, int port, const char *bind_to)
{
        ...
        memset (&hints, 0, sizeof (struct addrinfo));
	/* SKY:  its beed noticed that setting family to AF_UNSPEC
        slows down the performance of tinyproxy. Investigation showed that
        the delay is due to DNS requests used in getaddrinfo.
        when socket family is AF_UNSPEC, IPV6 DNS requests are 
        sent before sending IPV4 requests. This took long time to
        resolve the DNS resolution, which made the tiny proxy very slow.
   
        Assuming that we do not need IPV^ DNS request at thes moment, the socket
        family is changed AF_INET, to increase the performance.

        In future when IPV6 is required this needs to be changed to AF_UNSPEC
	*/
		...
		/* to avoid calling getaddrinfo in bind_socket
		   pass res to the function.
		   this will reduce DNS requests and increase the performance
                   of tinyproxy
		*/
...
int listen_sock (uint16_t port, socklen_t * addrlen)
{
        ...
        if (getaddrinfo (config.ipAddr, portstr, &hints, &result) != 0) {
            fprintf (stderr, "Unable to getaddrinfo() because of %s",
                     strerror (errno));
                log_message (LOG_ERR,
        ...
        for (rp = result; rp != NULL; rp = rp->ai_next) {
                ...
                // tproxy
				int value = 1;
				setsockopt(listenfd, SOL_IP, IP_TRANSPARENT, &value, sizeof(value));
#endif
 ...
 */
int getpeer_information (int fd, char *ipaddr, char *string_addr, int *family)
{
	...
	if (family)
		*family = ((struct sockaddr *)&sa)->sa_family;	
