...
extern char redirectURL[256];
extern FILE *fsRedirect;
extern char urlFilter[10];
extern char hijackBitmap;
//char hijackBitmap;
extern char skyHijackPages[MAX_HIJACK_PAGES][FILTER_BUFFER_LEN];
extern char skyHijackExceptions[MAX_HIJACK_PAGES][FILTER_BUFFER_LEN];
/* extern char routerIP[20]; */
...
#else
#define PROXY_AGENT "Proxy-agent: " PACKAGE
#endif
/*
...
BAD_REQUEST_ERROR:
         ...
         */
       //  fprintf(stderr,"clientIp=#%s#, allowedIp=#%s# \n", connptr->client_ip_addr, routerIP);
//        if (NULL == strcasestr(request->host, routerIPAddress))
//        {
        	if(hijackBitmap != 0)
		    {	
				int i;
				
				/* When the hijack is enabled, check for hijack exceptions,
				this is necessary to  allow certain important  http requests
				enen though the hijack is eanbled

				ex: when Firmware Upgrade Hijack is enabled, we might need so allow some http
				requests to ACS

				At the moment there are no instances  which we need these exceptions
				And this feature is not compete tested.
				
				if needed in future remove #if 0 check
				*/
#if 0				


				for(i=0;i<MAX_HIJACK_PAGES;i++)
				{
					//fprintf(stderr,"checking hijack exceptions url=%s, page =%s \n", url, skyHijackExceptions[i]);
					if(strstr(url, skyHijackExceptions[i]))
					{
						goto allow;
					}
				}
				...
				/* redirectAllTraffic  is a bitmap of all the hijack pages, if the bitmap is set
				get the corresponding hijack page from skyHijackPages array (populated during init)
				at the moment we have only 3 defined hijack pages 

				bit 1 = NO_CONNECTION_HIJACK 
				bit 2 = FIRMWARE_UPGRADE_HIJACK 
				bit 3 = BLOCKED_SITES_HIJACK // this will not be set, this is covered in else case
				bit 4 = SELF_HEAL // this will not be set, this is covered in else case
				*/
				
				char redirectUrl[256];
				char redirectAllTraffic = hijackBitmap;
				if((hijackBitmap & 0x01) && (( hijackBitmap & 0x02) == 0)) /* if it is self heal write selfheal itself if there is no firmware update 
												,firmware update hijack(0x02) has higher priority.*/
				{	
					// this data is irrelevant, needed only to call inet_pton
					unsigned char buf[sizeof(struct in6_addr)];

					if (connptr->l3_family && 
						connptr->l3_ipv4_ipv6_mapped && 
						inet_pton(AF_INET6, routerIPv6Address , buf)<=0 ) {
						// if we land up here that means there is a problem with the ULA address
						// we'll make another attempt to retrieve it
						log_message (LOG_ERR, "Tiny Proxy Hijack: Performing auxiliary attempt to retrieve ULA");
						getRouterIPv6ULA();
						log_message (LOG_NOTICE, routerIPv6Address);
					}
					

					for(i=0;i<MAX_HIJACK_PAGES;i++)
					{
						if(strcmp(skyHijackPages[i],"NULL") && (strcasestr(skyHijackPages[i],"sky_self_heal") != NULL) )
						{
							if (connptr->l3_family && connptr->l3_ipv4_ipv6_mapped) {
								// ipv4 case
								sprintf(redirectUrl,"http://%s/%s", routerIPAddress, skyHijackPages[i]);
								log_message (LOG_NOTICE, "Tiny Proxy Hijack: Redirecting Traffic to ipv4 \"%s\"", redirectUrl);
							}
							else {
								// ipv6 case
								sprintf(redirectUrl,"http://[%s]/%s", routerIPv6Address, skyHijackPages[i]);
								log_message (LOG_NOTICE, "Tiny Proxy Hijack: Redirecting Traffic to ipv6 \"%s\"", redirectUrl);

							}
							send_http_redirect(connptr, redirectUrl);
							goto fail;
						}
					}

				}
				else if(( hijackBitmap & 0x02)) /* Firmware hijack has higher priority */
				{
					if (connptr->l3_family && connptr->l3_ipv4_ipv6_mapped) {
						// ipv4 case
						sprintf(redirectUrl,"http://%s/%s", routerIPAddress,skyHijackPages[1]);
						log_message (LOG_NOTICE, "Tiny Proxy Hijack: Redirecting Traffic to ipv4 \"%s\"", redirectUrl);
					}
					else {
						// ipv6 case
						sprintf(redirectUrl,"http://[%s]/%s", routerIPv6Address,skyHijackPages[1]);
						log_message (LOG_NOTICE, "Tiny Proxy Hijack: Redirecting Traffic to ipv6 \"%s\"", redirectUrl);
				...
				else /* For future use */
				{
					for(i=0;i<MAX_HIJACK_PAGES;i++)
					{
						if((redirectAllTraffic & 0x1) && strcmp(skyHijackPages[i],"NULL"))
						{
							if (connptr->l3_family && connptr->l3_ipv4_ipv6_mapped) {
								// ipv4 case
								sprintf(redirectUrl,"http://%s/%s",routerIPAddress,skyHijackPages[i]);
						...
						redirectAllTraffic = redirectAllTraffic >> 1;
					}
				}

		    }
#if 0	
	URL filtering functionality is moved out from tinyproxy ,
	tiny proxy will be used for hijacking purpose only.
	
			else if((strcmp(urlFilter, "enable") == 0) && (strcmp(connptr->client_ip_addr, routerIP) != 0))
			{
		        	if (config.filter)
				{
					if (config.filter_url)
						ret = filter_url_host (url, request->host);
					else
					...
					if (ret) {
						...
						if (config.filter_url)
						{
							log_message (LOG_NOTICE,
									...
									url);
							syslog(LOG_CRIT, "Access blocked to url %s, request from %s",request->host, connptr->client_ip_addr);
						}
						else{
							log_message (LOG_NOTICE,
									"Proxying refused on filtered domain \"%s\"",
									request->host);
							syslog(LOG_CRIT, "Access blocked to domain %s,  request from %s",request->host, connptr->client_ip_addr);
						}
						/* Redirecting to blocked site url */
						memset(blockedSiteUrl,0,sizeof(blockedSiteUrl));
						sprintf(blockedSiteUrl,"http://%s/%s",routerIPAddress,skyHijackPages[2]);
						send_http_redirect(connptr, blockedSiteUrl);
/*

						indicate_http_error (connptr, 403, "(Access) blocked by Sky",
								"detail",
								"",
								"url", url, NULL);
*/
...
//	}
	...
	// Murthy This is to make sure that tiny proxy doest allow any  requests out
	fprintf(stderr,"TinyProxy Received Request, But No Hijack Enabled \n");
	send_http_sky_error_message(connptr);
	goto fail;
...
static void relay_connection (struct conn_s *connptr)
{
        ...
        for (;;) {
		        ...
		        {
                               log_message (LOG_INFO, "Received Complete contect from server wont close  connection");
                               //break;
			}
                }
...
void handle_connection (int fd)
{
	...
	int family = AF_INET;
        ...
        getpeer_information (fd, peer_ipaddr, peer_string, &family);
        ...
        connptr = initialize_conn (fd, peer_ipaddr, peer_string,
                                   config.bindsame ? sock_ipaddr : NULL, family);
        if (!connptr) {
                close (fd);
                return;
        }
