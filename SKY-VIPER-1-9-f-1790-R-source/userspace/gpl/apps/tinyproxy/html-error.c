...
int send_http_redirect (struct conn_s *connptr, const char *url)
{
        ...
        return (write_message (connptr->client_fd, headers, url));
}
...
int send_http_headers (struct conn_s *connptr, int code, const char *message)
{
	...
	sprintf(protocol,"HTTP/%d.%d",connptr->protocol.major, connptr->protocol.minor);
        return (write_message (connptr->client_fd, headers, protocol,
#ifndef SKY
...
int send_http_sky_error_message(struct conn_s *connptr)
{
        char protocol[10];
		...
		const char *body=
            ...
            "\"http://www.w3.org/TR/xhtml11/DTD/xhtml11.dtd\">\n"
            "<html>\n"
			"Tinyproxy is trying to send packets out, should investigate.\n"
			"To fix it temporarly, restart the router \n"	
            "</html>\n";

	       return (write_message (connptr->client_fd, headers, body));
}
