...
#ifdef SKY
#define RADVD_PREFIX_LEN_STD         (64)    /* The standard prefix length used in Router Advertisements */
#endif /* SKY */
...
#ifdef SKY
int get_IfEntry (const char *pIfName, struct Interface **pIfEntry);
#endif /* SKY */
...
/* send.c */
#ifdef SKY
int send_ra(struct Interface *iface, struct in6_addr *dest, uint8_t is_reset_lft);
int send_ra_for_if(const char *pIfName);
#else
