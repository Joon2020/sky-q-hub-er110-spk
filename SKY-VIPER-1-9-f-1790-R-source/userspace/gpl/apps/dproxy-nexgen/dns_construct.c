...
#ifdef SKY_IPV6  
  /* 11/06/2013: supporting DNC reply for AAA records queries,
        jira - ROUTER-2521
        */
  else if( m->message.question[0].type == AAA ){
	struct in6_addr in;
	inet_pton( AF_INET6, m->ip, &in );
	SET_UINT16_TO_N( m->here, 0xc00c, m->numread ); /* pointer to name */
	SET_UINT16_TO_N( m->here, AAA, m->numread );      /* type */
	SET_UINT16_TO_N( m->here, IN, m->numread );     /* class */
	if(m->ttl > 0) {
		SET_UINT32_TO_N( m->here, m->ttl, m->numread );  /* ttl */
	}
	...
	SET_UINT16_TO_N( m->here, 16, m->numread );      /* datalen */
	memcpy( m->here, &in.__in6_u, sizeof(in.__in6_u) ); /* data */
	m->numread += sizeof( in.__in6_u);
  }
#endif //  SKY_IPV6
}
/*****************************************************************************/
