...
void dns_probe_set_recover_time(void)
{
	...
	if (!strcmp(config.name_server, primary_ns)) /* change to seconday server */
	{
		...
		if (secondary_ns[0]) {
			...
			printf("%s Primary DNS server (%s) Is Down..."
					...
					date_time_buf, primary_ns, secondary_ns);
	...
	else if (!strcmp(config.name_server, secondary_ns))
	{
		dns_probe_switchback();/* switch back to primary since secondary is down*/
	}
	else
	{
		printf("\n DNS server name corrupted\n");
	}
