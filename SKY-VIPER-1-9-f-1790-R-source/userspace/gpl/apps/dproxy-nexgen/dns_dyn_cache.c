...
static char skyDomainName[32];
/*
	To have DNS resolution working for both DHCP and Static IP assignemnt for LAN Hosts 
	function dns_dyn_list_find_sky() is added to compare the DNS query name with either 
	Hostname format  or  Hostname.DomainName format
*/
	
/*****************************************************************************/
dns_dyn_list_t *dns_dyn_list_find_sky(dns_dyn_list_t *list, char *cname,
				  struct in_addr *addr)
{
  dns_dyn_list_t *ent;
  ...
  for(ent = list; ent; ent = ent->next) {
    sprintf(qualifiedName, "%s.%s",ent->cname, skyDomainName);
    if(cname && (strcasecmp(ent->cname, cname) != 0 ) && (strcasecmp(qualifiedName, cname) != 0))
	continue;
...
dns_dyn_list_t *dns_dyn_find(dns_request_t *m)
{
 ...
 /* SKY:
 	DNS_DYN_CACHE is used only to store hostnames in the local LAN network.
 	we will not cache host names from DNS responses dns_dyn_cache_add is commented
 	in dproxy.c.

       Router as .Home as the Domain name, and is send in DHCP  Domain Name option. 
       If Lan hosts gest IP address using DHCP, they will have .Home configured as Domain.
       LAN hosts will append Domain Name to Host Name in DNS query request. 
       i.e DNS query will be in the form of Hostname.DomainName

       If LAN host is using static IP address, then the DNS query will have only Hostname.
       
 	Hostnames in /var/hosts  file contains only host names (doesnt include domain name)

	To have DNS resolution working for both modes of IP address assignement, 
	dns_dyn_list_find_sky is added to compare the DNS query name with either Hostname format
	or Hostname.DomainName format
	
	
 */
      
   while((ret = dns_dyn_list_find_sky(dns_dyn_hosts, cname, paddr))) {
      goto found;
  }
...
// Extract Domain Name
     ...
     if (!(dnsFp = fopen(STATIC_DNS_FILE_DEFAULT, "r"))) {
    debug("can not open file %s", STATIC_DNS_FILE_DEFAULT);
    sprintf(skyDomainName, "Home");
  }
   
   while(fgets(line, BUF_SIZE, dnsFp)){
    line[strlen(line) - 1] = 0; /* get rid of '\n' */
    pToken = strtok( line, " \t");
    if(pToken == NULL) /* ignore blank lines */
      continue;
    if(pToken[0] == '#') /* ignore comments */
      continue;
    while((dName = strtok( NULL, " \t" ))){
      if(dName[0] == '#')
	break;
      if(strcmp(pToken,"domain")	  == 0)
      {
	     sprintf(skyDomainName, "%s",dName);
            break;
      	}
    }
  }
