...
int main (int argc, char *argv[]) {
	...
	printf("Mac Address       IP-Address      Expires %s      Hostname\n", mode == REMAINING ? "in" : "at");  
	...
	while (fread(&lease, sizeof(lease), 1, fp)) {
		...
		if (mode == REMAINING) {
			if (!expires) printf("expired, ");
				...
				if (expires > 60*60*24) {
					printf("%ld d, ", expires / (60*60*24));
					expires %= 60*60*24;
				}
				if (expires > 60*60) {
					printf("%ld h, ", expires / (60*60));
					expires %= 60*60;
				}
				if (expires > 60) {
					printf("%ld mins, ", expires / 60);
					expires %= 60;
				}
				printf("%ld sec, ", expires);
			}
		} else printf("%s", ctime((time_t *)&expires));

		printf(" %s\n", strlen(lease.hostname) ? lease.hostname : "UNKNOWN");
	}
	fclose(fp);
