...
void send_lease_info(UBOOL8 isDelete, const struct dhcpOfferedAddr *lease)
{
   ...
   if((lease->hostname == NULL) || (lease->hostname[0] == '\0'))
	   snprintf(body->hostName, sizeof(body->hostName), "UNKNOWN");
   else
   ...
   if(lease->classid)
	   sprintf(body->clientid,"%s",lease->clientid);
   else
	   sprintf(body->clientid,"%s"," ");
