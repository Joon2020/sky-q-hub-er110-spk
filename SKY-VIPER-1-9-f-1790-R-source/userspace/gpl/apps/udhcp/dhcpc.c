...
#ifdef SKY_EXTENDER
/* Shashank - 13/05/2014 - Extender: Defined a new message handle for the
 * DHCP client as the server and client can build independently of each other.
 */
static void *msgHandle_udhcpc = NULL;
#else
/* For non-extender, dhcp client doesn't need a new message handle.
 * Since dhcpc and dhcpd are actually the same binary, the msgHandle
 * is declared in dhcpd.c and used by dhcpc.
 */
#define msgHandle_udhcpc msgHandle
#endif // SKY_EXTENDER
static int state;
...
void sendEventMessage(UBOOL8 assigned, UBOOL8 isExpired, DhcpcStateChangedMsgBody *options)
{
   ...
   if ((ret = cmsMsg_send(msgHandle_udhcpc, msg)) != CMSRET_SUCCESS)
   {
...
{
#ifdef SKY_EXTENDER
    cmsMsg_cleanup(&msgHandle_udhcpc);
#endif
...
// brcm
   ...
   cmsMsg_initWithFlags(EID_DHCPC, EIF_MULTIPLE_INSTANCES, &msgHandle_udhcpc);
	...
	for (;;) {
        ...
        if ((old_mode != listen_mode) || (fd == -1))
        {
            ...
            if (dummy_fd >= 0)
            {
                close(dummy_fd);
                dummy_fd = -1;
                LOG(LOG_INFO, "**** Closed dummy socket ****");
            }
            ...
            if (listen_mode == LISTEN_RAW)
            {
                ...
                dummy_fd = dummy_socket(INADDR_ANY, CLIENT_PORT, client_config.interface);

                if (dummy_fd < 0)
                {
                    LOG(LOG_ERR, "Not Fatal. Just failed to open dummy socket. Expect ICMP port unreachable errors");
                }
                else
                {
                    LOG(LOG_INFO, "**** Created dummy socket ****");
                }
            }
        ...
        LOG(LOG_INFO, "*** fd = %d dummy_fd = %d state = %d listen mode = %d ***", fd, dummy_fd, state, listen_mode);
		...
		if (retval == 0) {
			...
			if ((message = (char *)get_option(&packet, DHCP_MESSAGE_TYPE, NULL)) == NULL) {
			...
			case INIT_SELECTING:
				...
				if (*message == DHCPOFFER) {
					if ((temp = (char *)get_option(&packet, DHCP_SERVER_ID, NULL))) {
			...
			case REBINDING:
				if (*message == DHCPACK) {
					if (!(temp = (char *)get_option(&packet, DHCP_LEASE_TIME, NULL))) {
			...
			case BOUND:
                if (*message == DHCPACK) {
                    LOG(LOG_ERR, "Ignoring DHCPACK in bound state");
                }
                break;
			case RELEASED:
				/* ignore all packets */
			...
			/* a signal was caught */
			LOG(LOG_ERR, "Caught a signal");
		} else {
			/* An error occured */
			LOG(LOG_ERR, "Error on select %d : %s", errno, strerror(errno));
		}
	}
	return 0;
}
