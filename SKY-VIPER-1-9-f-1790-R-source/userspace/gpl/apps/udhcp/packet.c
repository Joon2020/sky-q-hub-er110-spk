...
int get_packet(struct dhcpMessage *packet, int fd)
{
	...
	if (packet->op == BOOTREQUEST && (vendor = (char *)get_option(packet, DHCP_VENDOR, NULL))) {
		   ...
		   u_int32_t dest_ip, int dest_port, char *dest_arp, int ifindex)
{
	...
	if (!payload)
	{
		cmsLog_error( "payload not provided");
		return -1;
	}
	if (!dest_arp)
	{
		cmsLog_error( "dest_arp not provided");
		return -1;
	}

	if ((fd = socket(PF_PACKET, SOCK_DGRAM, htons(ETH_P_IP))) < 0) {
		cmsLog_error( "socket call failed: errno=%d", errno);
		return -1;
	}
	...
	if (bind(fd, (struct sockaddr *)&dest, sizeof(struct sockaddr_ll)) < 0) {
		cmsLog_error( "bind call failed: errno=%d", errno);
		close(fd);
		return -1;
	}
	...
	if (result <= 0) {
		cmsLog_error( "write on socket failed: errno=%d", errno);
	}
	close(fd);
	cmsLog_debug("DHCP packet sent result: %d\n", result);
	return result;
}
