...
unsigned char *get_option(struct dhcpMessage *packet, int code, int *optionLength)
{
	int i, length;
	...
	while (!done) {
		...
		if (optionptr[i + OPT_CODE] == code) {
			...
			if (optionLength) *optionLength= (int) *(optionptr+1+i);
...
int add_option_string(unsigned char *optionptr, unsigned char *string)
{
	...
	cmsLog_debug( "adding option 0x%02x", string[OPT_CODE]);
...
int add_simple_option(unsigned char *optionptr, unsigned char code, u_int32_t data)
{
	...
	cmsLog_debug( "adding option 0x%02x", code);
...
void attach_option(struct option_set **opt_list, struct dhcp_option *option, char *buffer, int length)
{
	...
	if ((existing = find_option(*opt_list, option->code))) {
		cmsLog_debug( "Attaching option %s to existing member of list", option->name);
		...
		cmsLog_debug( "Attaching option %s to list", option->name);
...
#ifndef SKY_EXTENDER // DHCPD related code
