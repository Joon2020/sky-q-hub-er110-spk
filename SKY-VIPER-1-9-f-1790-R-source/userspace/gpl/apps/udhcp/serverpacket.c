...
void skyDiagLogDhcpMsg(int msg, char *macAddr);
...
int sendOffer(struct dhcpMessage *oldpacket)
{
	...
	if(!static_lease_ip) {
        	...
        	} else if ((req = (char *)get_option(oldpacket, DHCP_REQUESTED_IP,NULL)) &&
		...
		/* otherwise, find a free IP */
        	...
        	if ((lease_time = (char *)get_option(oldpacket, DHCP_LEASE_TIME,NULL))) {
        ...
        if ((req = (char *)get_option(oldpacket, DHCP_VENDOR_IDENTIFYING,NULL))) {
...
#ifndef SKY_DIAGNOSTICS		
        return send_packet(&packet, 0);
#else  //SKY_DIAGNOSTICS
		int ret = send_packet(&packet, 0);
		if(ret)
			skyDiagLogDhcpMsg(DHCPOFFER, &packet.chaddr);
...
int sendNAK(struct dhcpMessage *oldpacket)
{
		...
		if(ret)
			skyDiagLogDhcpMsg(DHCPNAK, &packet.chaddr);
...
int sendACK(struct dhcpMessage *oldpacket, struct dhcpOfferedAddr *lease)
{
        struct dhcpMessage packet;
        struct option_set *curr;
...
#ifdef SKY_DIAGNOSTICS		
		skyDiagLogDhcpMsg(DHCPACK, &packet.chaddr);
#endif //SKY_DAIGNOSTICS
        ...
        vendorid = (char *)get_option(oldpacket, DHCP_VENDOR,NULL);
        userclsid = (char *)get_option(oldpacket, DHCP_USER_CLASS_ID,NULL);
        clientid = (char *)get_option(oldpacket, DHCP_CLIENT_ID,NULL);
        memset(offerlist->classid, 0, sizeof(offerlist->classid));
        memset(offerlist->vendorid, 0, sizeof(offerlist->vendorid));
        memset(lease->clientid, 0, sizeof(lease->clientid));
        if( vendorid != NULL){
 	     length = *(vendorid - 1);
