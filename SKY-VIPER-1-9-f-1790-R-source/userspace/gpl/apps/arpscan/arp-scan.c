...
static char hosts_savefile[MAXLINE];	/* pcap savefile filename */
FILE *hostsfd = NULL;
...
#ifdef SKY   
   unsigned char dest_mac_string[18]="";
   unsigned char dest_mac_sky[6] = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
   char host_sky[ARP_SCAN_IP_ADDR_LEN]="";
#endif /* SKY */   
/*
...
#else /* SKY */
		sscanf(line, "%s %s",host_sky, dest_mac_string);
//		printf("host_sky - %s, dest_mac_sky - %s \n",host_sky, dest_mac_sky);
		get_ether_addr(dest_mac_string, dest_mac_sky);
		add_host_pattern(host_sky, timeout, dest_mac_sky);
...
#else /* SKY */
	 add_host_pattern(localnet_descr, timeout, dest_mac_sky);
#endif /* SKY */
...
#else /* SKY */
	 add_host_pattern(*argv, timeout, dest_mac_sky);
#endif /* SKY */
         argv++;
      }
   }
 ...
 */
   if (*hosts_savefile != '\0') {
   	if ((hostsfd = fopen(hosts_savefile, "w")) == NULL) {
	      warn_sys("WARNING: Cannot open Hosts_savefile %s", hosts_savefile);
	      return 0;
	   }
   }
 ...
 */
         ...
         if (host_timediff >= (*cursor)->timeout) {
            ...
            if (debug) {print_times(); printf("main: Can send packet to host %s now.  host_timediff=%lu, timeout=%u, req_interval=%d, cum_err=%d\n", my_ntoa((*cursor)->addr), host_timediff, (*cursor)->timeout, req_interval, cum_err);}
 ...
 */
            ...
            if (debug) {print_times(); printf("main: Can't send packet to host %s yet. host_timediff=%lu\n", my_ntoa((*cursor)->addr), host_timediff);}
         } /* End If */
...
#else /* SKY */
   if(he)
	   memcpy(frame_hdr.dest_addr, he->dest_mac, ETH_ALEN);
   else
	   memcpy(frame_hdr.dest_addr, target_mac, ETH_ALEN);
#endif /* SKY */
...
usage(int status) {
   ...
   fprintf(stderr, "\n--hostsavefile=<p> or -W <p>\tWrite output to file <p>.\n");
   fprintf(stderr, "\n");
 ...
 */
#ifndef SKY 
void add_host_pattern(const char *pattern, unsigned host_timeout) 
#else /* SKY */
void add_host_pattern(const char *pattern, unsigned host_timeout, const char *dest_mac_sky) 
#endif /* SKY */
{
   char *patcopy;
...
#else /* SKY */
		add_host(ipstr, host_timeout, NULL);
#endif /* SKY  */		 
...
#else /* SKY */
	  add_host(patcopy, host_timeout, dest_mac_sky);
#endif /*SKY */
   }
   free(patcopy);
}
 ...
 */
#ifndef SKY
void add_host(const char *host_name, unsigned host_timeout) 
#else /* SKY */
void add_host(const char *host_name, unsigned host_timeout, const char *dest_mac_sky) 
#endif /* SKY */
{
...
#ifdef SKY   
   if(dest_mac_sky != NULL)
   {
   		memcpy(he->dest_mac, dest_mac_sky, ETH_ALEN);
		//printf("add_host , dest_mac_sky - %s \n",dest_mac_sky);
   }
#endif   
}
...
process_options(int argc, char *argv[]) {
   struct option long_options[] = {
      ...
      {"hostssavefile", required_argument, 0, 'z'},
      {0, 0, 0, 0}
   };
   const char *short_options =
      "f:hr:t:i:b:vVdn:I:qgRNB:O:s:o:H:p:T:P:a:A:y:u:w:S:F:m:lLQ:W:z:";
   int arg;
   int options_index=0;
   ...
   while ((arg=getopt_long_only(argc, argv, short_options, long_options, &options_index)) != -1) {
      switch (arg) {
         ...
         case 'W':	/* --pcapsavefile */
            ...
            break;
         case 'z':	/* --pcapsavefile */
            strlcpy(hosts_savefile, optarg, sizeof(hosts_savefile));
            break;
