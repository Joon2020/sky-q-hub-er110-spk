...
#ifdef BUFFER_HACK
	  wrq.u.data.flags = IW_SCAN_BUFFER_HACK;
	  newbuf = realloc(buffer, buflen + IW_SCAN_MAX_DATA + 1);
#else
...
#ifdef BUFFER_HACK
      iw_init_event_stream(&stream, (char *) buffer, wrq.u.data.length + (wrq.u.data.flags & IW_SCAN_BUFFER_HACK ? IW_SCAN_MAX_DATA + 1 : 0));
#else
